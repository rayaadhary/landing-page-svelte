...
<div class="space-y-6">
	<div class="flex items-center justify-between">
		<h1 class="text-2xl font-black text-slate-900">Produk</h1>
		<button onclick={addNew} class="rounded-lg bg-[#0155FF] px-4 py-2 text-sm font-bold text-white hover:opacity-90">+ Tambah Produk</button>
	</div>
			...
			{#each items as item}
				<div class="flex items-center justify-between rounded-xl border border-slate-200 bg-white p-4">
					<div>
						...
						<p class="text-xs text-slate-500">{item.category} · {item.slug}</p>
					</div>
